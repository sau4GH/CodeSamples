...
public class PrintMatrixInGivenOrder {
	...
	public static void solution_4(int[][] matrix) {
		...
		while (i < matrixSize) {
			...
			int columnIndex = i % (matrixColumnSize * 2);
			
			int columnIndexToBeUsed  = columnIndex < matrixColumnSize ? (Integer) stack.push(columnIndex) : (Integer) stack.pop();			
			
			//System.out.println(rowIndex + " " + columnIndexToBeUsed);
